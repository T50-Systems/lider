...
**Three run kinds.** `construction` (default) is the build/ship graph. `inception` is optional discovery that seals `.lider/handoffs/` for construction to `import`. `operations` is optional shared/deployed-state work: pin `target`, record ternary preflight/effect checks, walk scope→preflight→act→prove→soak?→closed. Inception and operations are **recommended** (not required); `init --strict` / `LIDER_STRICT=1` tightens gates (challenge+handoff; preflight before act and effect before closed). How to preflight/verify stays in those skills — the ledger only refuses silent skips.

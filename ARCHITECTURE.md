...
**Multi-harness.** Skills install under **Claude Code** and **Grok Build** via marketplace plugins, and under **OpenCode / Pi / Codex** via `install-skills.py` into `.agents/skills`, `.opencode/skills`, `.pi/skills`, `.codex/skills`. Plugin root: `LIDER_PLUGIN_ROOT` / `GROK_PLUGIN_ROOT` / `CLAUDE_PLUGIN_ROOT` (see `lider.root.plugin_root()`). **Engine adapters** (what the runtime supervises): `claude`, `grok`, `codex`, `opencode`, `pi`, `calvoproxy`, `generic`. Cross-engine review is *implementer family ≠ reviewer family* (runtime id for opencode/pi).

**Three run kinds.** `construction` (default) is the build/ship graph. `inception` is optional discovery that seals `.lider/handoffs/` for construction to `import`. `operations` is optional shared/deployed-state work: pin `target` (with optional `previous_ref`), walk scope→preflight→act→prove→soak?→closed, and on failure **incident→rollback|act→prove**. Inception and operations are **recommended** (not required); `init --strict` / `LIDER_STRICT=1` tightens gates. How to preflight/verify stays in those skills — the ledger records ternary checks and refuses silent skips (including incident without a failure signal and rollback without previous_ref).

**Prompt graph engineering (G1–G4).** See [`docs/PGE.md`](docs/PGE.md). Structure = edge tables + guards; content = pinned specs + `templates/roles/*`; **G3 = ledger transition semantics, not an engine scheduler**; G4 artifacts include `run.json`, handoffs, and `rungraph snapshot`. Skills guide with prose; the ledger decides (`enter` / `assign`). Free-form “agents invent the flow” is out of bounds.
...
- **The spec is re-verified, not merely pinned.** `enter implement` re-hashes the file: changed since pinning is refused, unreadable is `undetermined`. It is the only Inception-side guard that checks a declaration against a fact from outside the ledger, which is why it was built first.
- **Open questions block as `undetermined`.** An unanswered input is an unestablished one, so it reuses the existing exit-code meaning instead of inventing a new one. An assumption may unblock, but only a recorded one (`--status assumed` requires `--answer`).
- **Coverage is a checked relation, and an honest one.** `enter plan` refuses while a required criterion is covered by no unit. The limit is stated in the refusal text and the docs: both sides are written by the same orchestrator, so this verifies bookkeeping consistency, not that anything was implemented. It catches a requirement dropped by never declaring a unit — real, common, otherwise invisible — and claims nothing further.
- **`next` is advisory.** It reports eligibility and records the concurrency width; it never decides or acts. The ledger stays an arbiter. Whether it should ever become a scheduler is a question deferred to that recorded data.
- **Units of work are subgraphs.** A phase that decomposes declares units; each runs its own `pending → implement → review → adjudicate → done` cycle with its own findings, rounds and convergence, because a unit carries the same shape as the run itself. Dependencies between units are enforced edges, and `join` is a barrier that will not open while any unit is open. A dropped unit is terminal but does not hide its findings — `verify` still refuses over an undecided BLOCKER inside one. A phase that is genuinely one unit declares nothing and takes the flat path.
